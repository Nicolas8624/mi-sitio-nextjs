  ...
  return (
    <section className="px-5 py-8">
      <div className="max-w-xl mx-auto">
        <h2 className="text-3xl font-bold text-center text-gray-900 mb-8">Contacto</h2>
        <form action={formAction} className="flex flex-col gap-4">
          ...
          {state.message && <p className={`text-center font-medium ${state.success ? "text-green-600" : "text-red-500"}`}>{state.message}</p>}
        </form>
      </div>
    </section>
  );
